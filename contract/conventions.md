...
- 图片请求中的 `frameEdit` 是本次编辑的恢复快照，和生成任务一起保存。受理时还需 `agent:run` 权限；仅对话属主可编辑，并校验目标文件、镜头组、参考帧仍指向 `sourceUrl`。目标已改变返回 `409`，不创建付费生成任务。
- 用户控制提交图片的内容与顺序。`frameEdit.references` 必须和 `referenceImageUrls` 按 URL 逐项一致；服务端不补充底图、标注图或重排图片。
- 标注和图片引用绑定稳定 ID。服务端在受理时把结构化修改要求编译成 `prompt`，按本次图片列表解析编号；标注引用必须有明确选择的标注图。标注只帮助模型理解位置，不提供区域外像素不变的保证。
- 标注点数按工具校验：`point` 恰好一个定位点，`rectangle`、`ellipse`、`arrow` 恰好两个端点，`pen` 至少两个点；OpenAPI 的数组长度约束不表达这种按 `kind` 区分的关系。
- `GET /generations` 的类型、文件、镜头组和帧筛选在分页截断前执行，归属范围不因筛选扩大。使用上一页最后一项的 `id` 作为 `before` 继续读取；按创建时间与 ID 倒序，空列表表示读完。
