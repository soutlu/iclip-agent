...
- 分叉：衍生记录跟着根一起拷，原作号按新旧 id 对照换成新根；只有参考片段不拷（ADR-0029 §3）。
...
记录之间的引用要么走列（原作号），要么用拷贝时不会变的键。分叉会把整条链拷进副本并给每行换新 id，便签里写着的记录 id 会随之失效，而服务端不读写便签、改不了它。所以「这一版基于哪一版」在便签里用那一版的 `editId`（`baseEdit`，缺省即原片）来指，`editId` 是前端铸的、拷贝时原样带过去。以后新的加工类型同理：引用别的记录，走列或用不变的键。

### 5. 存量照抄

迁移 0013 把便签上能对上一条真实记录的 `rootJob` 抄进列，然后从便签里擦掉；迁移 0014 把 `baseJob` 按它指向谁改写——指向根就擦掉（基于原片不写），指向成片就换成那条成片的 `editId`（`baseEdit`），不留旧键。对不上号的、原作号指向衍生记录的、没原作号的 clip，任一存在就带 id 报错终止：静默留成独立记录会让它们进审计、进分叉拷贝。降级把列写回便签再删列。
...
- **接受**：分叉拷贝要先在进程里给选中的行配好新 id 再写一条 `INSERT … SELECT`，比原来纯 SQL 多一次往返。出片记录上百条的量级，无所谓。
