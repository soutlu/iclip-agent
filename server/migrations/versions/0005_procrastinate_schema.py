...
revision: str = "b7f3c1d90a24"
down_revision: str | None = "e5b92c4718af"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None

# procrastinate 3.9.0 procrastinate/sql/schema.sql 原文。
_SCHEMA_SQL = r"""-- Procrastinate Schema
...
def upgrade() -> None:
    statements = _statements(_SCHEMA_SQL)
    if len(statements) != _STATEMENT_COUNT:
...
def downgrade() -> None:
    ...
    op.execute("DROP FUNCTION IF EXISTS procrastinate_fetch_job_v2(character varying[], bigint)")
    op.execute(
        "DROP FUNCTION IF EXISTS procrastinate_finish_job_v1(bigint, procrastinate_job_status, boolean)"
    )
    op.execute("DROP FUNCTION IF EXISTS procrastinate_cancel_job_v1(bigint, boolean, boolean)")
    op.execute(
    ...
    op.execute("DROP FUNCTION IF EXISTS procrastinate_unregister_worker_v1(bigint)")
    op.execute("DROP FUNCTION IF EXISTS procrastinate_update_heartbeat_v1(bigint)")
    op.execute("DROP FUNCTION IF EXISTS procrastinate_prune_stalled_workers_v1(float)")
