...
class GenerationService:
    ...
    async def submit(self, principal: Principal, request: GenerationRequest) -> GenerationJob:
        ...
        if isinstance(request, VideoGenerationIn):
            ...
            request = self._settle_image_model(request)
            provider = request.model or self._image_default_model
    ...
    def _settle_image_model(self, request: ImageGenerationIn) -> ImageGenerationIn:
        """把这次要用哪家、哪个渠道定下来写进请求。
        ...
        return request.model_copy(update={"model": model, "channel": channel})
