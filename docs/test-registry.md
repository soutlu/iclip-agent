...
| T-UPLOAD-01 | unit | `POST /uploads`：401/403（要 `assets:write`）；同内容落同一个对象 key（文件名换了也一样）；后缀取自 content type，客户端文件名进不了 key；白名单外的类型、空文件、超上限一律 422 且**不碰存储**；存储写不进是 502 | 命名权外流 / 拒了还是写了进去 / 存储故障报成自己的 bug |
