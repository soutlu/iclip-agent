...
const originalDocument = (): ShotsDocument => ({
  aspectRatio: '9:16',
  shots: [{ index: 1, imageUrls: [ORIGINAL], prompt: '原有描述 @Image1。', seconds: 6 }],
})
...
const completedJob = (outputUrl: string, createdAt: string): GenerationJob => ({
  ...
  request: {
    frameEdit: {
      ...
      frameNumber: 1,
      annotations: [],
      instructions: [{ kind: 'text', text: '修改颜色' }],
      references: [{ id: 'original', kind: 'image', url: ORIGINAL, label: '原图' }],
    },
  },
})
...
const provideJobs = () => {
  ...
  server.use(
    ...
    http.get('*/api/conversations/:id/workspace/files', () => HttpResponse.json({ files: [] })),
  )
}
...
describe('图片编辑结果应用', () => {
  ...
  it('应用 A 保存失败后恢复原图，保留描述并允许直接改选 B 保存', async () => {
    ...
    server.use(
      ...
      http.put('*/api/conversations/:id/workspace/file', async ({ request }) => {
        ...
        if (document.shots[0]?.imageUrls[0] === CANDIDATE_A) {
          await savingA
    ...
    const description = within(page).getByRole('textbox', { name: '镜头组 1 的描述' })
    description.focus()
    ...
    try {
      await waitFor(() =>
        expect(writes.some((item) => item.shots[0]?.imageUrls[0] === CANDIDATE_A)).toBe(true),
      )
      expect(screen.getByRole('dialog', { name: '编辑图片' })).toBeVisible()
      expect(persisted.shots[0]?.imageUrls).toEqual([ORIGINAL])
    } finally {
      release()
    }
    ...
    expect(persisted.shots[0]?.imageUrls).toEqual([CANDIDATE_B])
    expect(persisted.shots[0]?.prompt).toContain('同时修改的描述')
    expect(within(page).getByRole('img', { name: '镜头组 1 第 1 帧' })).toHaveAttribute(
      'src',
      CANDIDATE_B,
    )
    expect(writes.filter((item) => item.shots[0]?.imageUrls[0] === CANDIDATE_A)).toHaveLength(1)
  })
  ...
  it('编辑器打开后原图被外部修改时，拒绝应用并保留外部图片', async () => {
    ...
    persisted = {
      ...persisted,
      shots: persisted.shots.map((shot) => ({ ...shot, imageUrls: [externalUrl] })),
    }
    version += 1
    await act(() =>
    ...
    expect(persisted.shots[0]?.imageUrls).toEqual([externalUrl])
    expect(within(editor).getByRole('img', { name: '图片编辑结果' })).toHaveAttribute(
      'src',
      CANDIDATE_A,
    )
