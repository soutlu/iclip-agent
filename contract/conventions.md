...
>
> **机器可读的那一半在 [`openapi.json`](openapi.json)**：由 `make contract` 从后端导出，前端 `pnpm contract:generate` 据此生成类型与 zod。端点、字段与状态码以它为准；本文写的是它表达不了的部分——路由代理、双主体认证、命名与错误信封这些跨端约定。两者不一致时以 openapi.json 为准，并回来修本文。
