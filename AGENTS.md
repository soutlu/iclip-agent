...
iclip-agent 是 Productor 视频创作产品的重写主体：模块化单体 monorepo，`server/`（Python 3.13 + uv，FastAPI）+ `web/`（Producer 前端纯拷贝迁入，**冻结**，重构另立轨道）。**Postgres 是唯一事实源**；认证双主体（登录用户 cookie + API 调用方 Bearer key）；Agent 引擎选定 PydanticAI，关在 harness 围栏内。

后端现有实现：identity 业务模块（双主体 + SSO/PMS + 统一权限模型 [ADR-0002](docs/adr/0002-unified-permission-model.md)）、配置系统、架构门禁、CI workflow；`harness/`（通用内核）与 `capabilities/`（能力包）为空包占位，围栏已生效。后续实现范围与顺序由用户决定。
...
- **Policy 执法归 harness**：账本资格 / 授权 / 审计由 harness 在装配时包在全部能力合并后的工具面外侧，不进任何能力包。
...
- 迁移期从新后端输出重新生成 golden fixtures——fixtures 是冻结验收标准。
- 修改 `web/`——它是只读 UI 参考稿；前端对接时点由用户决策。
- CORS 配 `"*"`。
...
| 依赖升级 | 引擎升级 PR 契约门禁全绿才可合，release notes 含行为开关时人工审阅；web 运行时依赖升级人工决策，升级后 `pnpm verify` 全绿再合入 |
...
| [docs/CONTEXT.md](docs/CONTEXT.md) | 领域锚点：术语、上下文、不变量、禁止逻辑（完整登记，含后端尚未实现部分） | 领域语言或不变量变化时 |
