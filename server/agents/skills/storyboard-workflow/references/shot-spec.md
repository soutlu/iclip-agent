...
- `global_settings`：参考锁定和剪辑形式。
- `timeline`：按镜头顺序排列，每镜一项。每项包含 `timestamps`（起止秒数）和 `prompt`（该镜正文）。

发给视频模型时，系统按 `global_settings` → 空行 → 每镜一行 `[起–止秒｜镜头N] 正文` → `不要生成字幕，不要生成背景音乐。` 拼成一段文本。镜头标记与末尾这句都由系统追加，不写进文件。
...
- 配乐与 BGM 后期铺，不写进 prompt：Storyline 里的配乐描述（鼓点渐入、推至高潮）在转写时丢弃。
...
交付前把每个镜头组的 prompt 当一条视频从头放一遍，看三件事：连戏——每镜结尾时人的姿势、产品的位置和状态，下一镜要从这里接着来；做得出——每个动作这个人用这个产品在这段时长里能完成（乐福鞋的「点按鞋垫」搬到及膝长靴上就不成立）；对得上——句子说的景别、动作与嵌在里面的镜头帧一致，台词装得进时长。有问题的句子改到三条都成立为止，帧与 `image_urls` 不动。
