...
`image_urls` = 该镜头组的全部镜头帧 URL，按帧号排序（先镜头号，同一镜头号内按序号）；URL 只收这段对话里出现过的地址；上下文里翻不到时用 `read_file` 读 `frames/grids/` 下的版记录取回来。按列表顺序在 prompt 中编号为 `@Image1`、`@Image2`……`@ImageN`，每个 `@ImageN` 都必须能映射到 `image_urls` 第 N 项。
