...
两列 Markdown 表格 `| 结构层级 | Storyline |`，行数、顺序、名称与第 2 节结构节点完全一致，行与行之间不空行。同一结构层级内的多个镜头写在同一单元格内，用 `<br><br>` 分隔。每个镜头按时间顺序写成一段：`**[00:00.000-00:03.800]**` 后先写与上一镜的切换方式（第一镜写「开场」）和景别，再一句话写机位在哪、固定还是怎么动（方向与快慢），然后写人物依次做的动作，边做边说的按「动作，说：{台词}」交替写，最后写背景元素与声音。

- 人物用第 3 节出场清单里的名称指代，不另起称呼；同一镜头内再次提及用代词即可。
- 台词一律用 `{ }` 包裹，前面写明语种与说话人，没有出镜说话人的旁白写明语种与音色，台词逐句转写并保持原始语言；不用「继续介绍…」这类转述代替原话。
- 动作按看到的逐个写，边做边说的按「动作，说：{台词}」交替写，不把一镜的台词攒成一段；「举着鞋展示」这类静态姿态不算动作，要写成举起、翻转、递近这样的具体动作。
...
| 结构层级 | Storyline |
| :--- | :--- |
| Rain-Step Hook | **[00:00.000-00:03.800]** 开场，中景，手持机位在她侧后方平视跟拍，深色长发年轻女性拉开门走出，回手带上门，抬脚踩上潮湿的人行道，背景为阴天街口、积水路面和远处树影，声音以环境雨声和轻微脚步声为主，阴天顶光散射，冷调，低反差。<br><br>**[00:03.800-00:05.600]** 硬切，特写，机位贴地固定、低角度拍鞋，鞋底压过积水路面（慢动作），水膜自鞋纹两侧挤出，英文女声旁白：{Wet ground is where most shoes give up}，背景为虚化的湿滑砖面与倒影，声音以踩水声和持续雨声为主，散射光延续，鞋面高光弱。<br><br>**[00:05.600-00:09.600]** 匹配剪辑，近景，机位正面平视，从鞋面缓慢上摇至她的面部后固定，她停下脚步转向镜头，用英语说：{These still grip right through it}，抬起右脚把鞋底朝向镜头，说：{Look}，背景为街口积水与远处车灯，雨声打底、脚步声停，鼓点在此处进入。 |
