...
| [../design-system.html](../design-system.html)                     | 全局视觉规范与 token 定义（在仓库根目录）：颜色、排版、尺度、交互状态、图标与复用原则；`:root` / `.dark` 两块是浅深两套 token 的唯一事实源，运行时镜像由 `pnpm lint:design` 对账 | token 家族或全局规则变化时   |
