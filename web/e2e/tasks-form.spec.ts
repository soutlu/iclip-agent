...
test('需求单完整创建回读：规格、商品图库、分类参考图和单视频', async ({ page }) => {
  ...
  await dialog.getByLabel('商品款号', { exact: true }).fill('QA-BOOTS-001')
  await dialog.getByLabel('商品名称', { exact: true }).fill('黑色短靴')
  await dialog
    .getByLabel('创作要求', { exact: true })
    .fill('展示短靴轮廓与皮革纹理，呈现简洁自然的日常穿搭。')

  const png = await productPng(page)
  const imageInput = dialog.getByLabel('选择商品图片文件', { exact: true })
  ...
  await expect(dialog.getByRole('button', { name: '预览商品图片 2', exact: true })).toBeVisible()
  await expect
    .poll(() =>
      dialog
        .getByAltText('商品图片 1', { exact: true })
  ...
  await page.getByRole('button', { name: /黑色短靴宣传视频/ }).click()
  ...
  await expect(reopened.getByLabel('商品款号', { exact: true })).toBeDisabled()
  await expect(reopened.getByLabel('商品名称', { exact: true })).toHaveValue('黑色短靴')
  await expect(reopened.getByRole('button', { name: '预览商品图片 2', exact: true })).toBeVisible()
})
...
test('手机需求单正文可滚动，长创作要求与固定操作栏可用', async ({ page }) => {
  ...
  await dialog.getByLabel('商品款号', { exact: true }).fill('QA-MOBILE-001')
