...
> server 与 web 之间 wire 契约的总约定。golden fixtures v2（尚未启用，见 §5）与本文件同为合同事实源：**改合同的一侧负责同步两端，任何一端门禁红即合同漂移**。
...
- 登录态唯一事实源：`GET /users/me` → `{ "user": { ...camelCase, roles[], directPermissions[], permissions[], city, jobTitle, departments[] } }`；401 表示未登录。（UI 参考稿仍读旧字段 `role`，前端对接时统一处理；见 [ADR-0002](../docs/adr/0002-unified-permission-model.md)）
...
## 5. golden fixtures v2（尚未启用）

- golden fixtures = 锁定为验收标准的事件流样本。fixtures = **JSON 事件序列数组**（传输中立）；SSE 帧格式、WS 信封各自只有独立的小编码测试。合同锁事件序列，不锁传输帧。
- 每条事件流必须独立通过 run 序列校验（RUN_STARTED 起、有且仅有一个终止事件、text/tool 配对闭合）。
- 套件随实现分批启用：首批为 new/replay/snapshot/cancel 子集；HITL fixture 形状先从旧系统捕获锁定，HITL 实现时启用。
- **严禁从新后端输出重新生成 fixtures**——迁移期 fixtures 是冻结验收标准。

## 6. 传输规划

命令面（run 发起 / resume / cancel / restore）**永久 HTTP**；事件投递首个版本为 SSE（`Accept: text/event-stream` 内联流，过渡方案），web 重构期切换统一 WS 网关（信封 `{channel, seq, payload}`，run 频道载荷为标准 AG-UI 事件原文），SSE 随后退役。同一 run 端点按 `Accept` 协商两种投递。
