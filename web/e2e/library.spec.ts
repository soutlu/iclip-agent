...
test('从侧栏进资料库，按画幅与关键词筛，筛选留在地址里，退回还原', async ({ page }) => {
  ...
  await expect(main.getByRole('article', { name: '跑鞋手持展示' })).toBeVisible()
  await expect(main.getByText('共 6 条')).toBeVisible()
  ...
  await expect(main.getByRole('article', { name: '滑板女孩 · 厚底靴街拍' })).toBeVisible()
})
