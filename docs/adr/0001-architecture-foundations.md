...
「不要造轮子」的另一面约定——以下**不是轮子**，不得以该指令为由替换：

1. **显式组合根**：模式不是框架；DI 容器才是非常规选择。
2. **Principal 解析器 + api_keys 表**：双主体没有被广泛采用的现成包；实现面 = 一张表 + 一个 dependency。
3. **StepPersistence 的 Postgres 后端**：官方只提供内存 / 文件 / SQLite / Mongo 后端，PG 后端必须自写（实现官方协议，不改表结构）。
