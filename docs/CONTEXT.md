...
1. **Postgres 是唯一持久化事实源。** 业务事实、agent 运行与消息历史都落 Postgres；worker 进程内存只承载当前连接、当前 run 或性能缓存，绝不承载跨 worker 正确性。
...
6. **run 生命周期由数据库持有。** 没有自定义 pending 运行时、没有本地 run 队列；运行事实先落库，恢复与续跑一律读库。
