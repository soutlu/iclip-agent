...
| T-SHOT-03 | unit | 逐格请求的校验与出图的重试升级：没有取帧账本即指向 `plan_shot_frames`；帧号形状错、同批重复、prompt 为空、条数不在 1-4 **都在提交之前**被拒（一次都没提交）；**帧号不必是账本里的候选帧**（定格只记属于哪一镜，新增或短于一秒的镜头没有候选帧也要出定格）；整版按 `4k` 提交且空格由中性面板补满；任何失败（连不上、5xx、被拒、结果不明、`OUTPUT_*` 等）都沿 dev,dev,pro 往下试到有图或试完，`pro_attempts` 为 0 时停在 dev,dev；**等待时限用尽不再提交下一个**；画幅不合规在提交之前拒；等超时也要把记录 id 报回去 | dev 一次失败就没图 / 时限已尽还在提交 / 一次生成没有结论 |
