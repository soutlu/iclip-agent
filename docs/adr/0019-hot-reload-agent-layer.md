...
结果写在 `GET /healthz` 的 `config` 段：`generation` 每次成功递增，`error` 是失败原因，`needs_restart` 为真表示改动落在非热区段。`status` 恒为 `ok`，探活与 compose 的健康检查不受重载结果影响。发布脚本只凭这一段判断成功、失败还是改走重启，不需要任何凭证。这段不鉴权：错误文案只写 agent id、模型名与环境变量名，不写任何取值。
