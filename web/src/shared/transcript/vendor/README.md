...
- 工具帧增加 `metadata`：声明位于 [model/frame.ts](model/frame.ts)，接收校验位于 [contract/schema.ts](contract/schema.ts)，相等判断位于 [ops/apply.ts](ops/apply.ts) 的 `frameEquals`，三处一起维护；它承载面向用户的工具结果。
